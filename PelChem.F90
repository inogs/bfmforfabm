...
! Initialize pelagic base model (this also sets the time unit to per day,instead of the default per second)
     ...
     ! Obtain the values of all model parameters from FABM.
      ...
      call self%register_diagnostic_variable(self%id_remX3c,    'remX3c',     'mgC/m3/d',  'remineralization of cdom X3c',output=output_none)
      call self%register_diagnostic_variable(self%id_remR3c,    'remR3c',     'mgC/m3/d',  'remineralization of dom R3c',output=output_none)      
