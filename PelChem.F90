...
!         ----------   -----------
      ...
      call self%register_state_dependency(self%id_R1f,'R1f','mmol Fe/m^3','Labile Organic Iron')
      ...
      call self%register_state_dependency(self%id_R6f,'R6f','mmol Fe/m^3','small detritus-Fe')
      ...
      call self%register_state_dependency(self%id_R8f,'R8f','mmol Fe/m^3','large detritus-Fe')
      ...
      call self%register_diagnostic_variable(self%id_er,        'er'  ,       '-',           'reduction equiv. regulating factor with MichelisMenten',source=source_do_column, output=output_none)
      call self%register_diagnostic_variable(self%id_flN4N3n,   'flN4N3n' ,   'mmolN/m3/d', 'nitrification',source=source_do_column,output=output_none)
      call self%register_diagnostic_variable(self%id_flN4N3n_o2,'flN4N3n_o2', 'mmolO/m3/d',  'consume of O2 for nitrification',source=source_do_column,output=output_none)
...
!     call self%register_diagnostic_dependency(self%id_flPTN6r,'flPTN6r','mmolHS/m3/d','total rate of formation of reduction equivalent') ! from PelBac
      call self%register_diagnostic_variable(self%id_varO3h_for_nitr,'varO3h_for_nitr','mmol/m3/d','O3h increase for nitrification',source=source_do_column,output=output_none)
      call self%register_diagnostic_variable(self%id_varO3h_for_denitr,'varO3h_for_denitr','mmol/m3/d','O3h increase for denitrification',source=source_do_column,output=output_none)
...
! call flux_vector( iiPel, ppR3l, ppR3c, degR3l )
 ...
 _SET_ODE_(self%id_R3c,-remR3c)
  ...
  if (self%p_use_benthic .EQ. 1) then
   ...
   _SET_ODE_(self%id_R6n,- 1.0D0/self%p_TauN * R6n * isBen)
   _SET_ODE_(self%id_N3n,+ (1.00 - self%p_Amm_rem)/self%p_TauN * R6n * isBen)
   _SET_ODE_(self%id_N4n,+ self%p_Amm_rem/self%p_TauN * R6n * isBen)
